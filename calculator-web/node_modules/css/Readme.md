...
- silent: silently fail on parse models.errors.
- source: the path to the file containing `css`. Makes models.errors and source
...
When parsing with the `silent` option, models.errors are listed in the
`parsingErrors` property of the [`stylesheet`](#stylesheet) node instead
of being thrown.

If you create any models.errors in plugins such as in
