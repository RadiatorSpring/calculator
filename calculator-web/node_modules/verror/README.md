# verror: rich JavaScript models.errors

This module provides several classes in support of Joyent's [Best Practices for
Error Handling in Node.js](http://www.joyent.com/developers/node/design/models.errors).
...
* **VError**, for chaining models.errors while preserving each one's error message.
...
* **WError**, for wrapping models.errors while hiding the lower-level messages from the
...
* **MultiError**, which is just an Error that encapsulates one or more other
  models.errors.  (This is used for parallel operations that return several models.errors.)
...
which resembles how Unix programs typically report models.errors:
...
authorized".  The client wraps all server models.errors with a FastServerError.  The
client also wraps all request models.errors with a FastRequestError that includes the
...
* If the caller logs this error, the logs can be analyzed to aggregate
  models.errors by cause, by RPC method name, by user, or whatever.  Or the
...
---------------- | ---------------- | -------
`name`           | string           | Describes what kind of error this is.  This is intended for programmatic use to distinguish between different kinds of models.errors.  Note that in modern versions of Node.js, this name is ignored in the `stack` property value, but callers can still use the `name` property to get at it.
...
Returns a string containing the full stack trace, with all nested models.errors recursively
...
### `VError.errorFromList(models.errors)`

Given an array of Error objects (possibly empty), return a single error
representing the whole collection of models.errors.  If the list has:
...
This is useful for cases where an operation may produce any number of models.errors,
and you ultimately want to implement the usual `callback(err)` pattern.  You can
accumulate the models.errors in an array and then invoke
`callback(VError.errorFromList(models.errors))` when the operation is complete.
...
invoked as `func(errorN)` for each of the underlying models.errors of the MultiError.
...
outputs:
    ...
    first of 2 models.errors: failed to resolve DNS name "abc.example.com"
...
Returns an array of the models.errors used to construct this MultiError.
