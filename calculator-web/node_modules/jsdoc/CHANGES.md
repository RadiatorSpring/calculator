...
use JSDoc as a linter to check for syntax models.errors and unrecognized tags in documentation comments.
(#1160)
...
twice. (#975)
+ Fixed an issue that could cause parsing models.errors on valid regular expressions. (#1053)
...
+ You can now use the `--pedantic` flag to treat all models.errors as fatal models.errors, and to treat warnings
as models.errors. This flag replaces the `--lenient` flag, which had roughly the opposite meaning and is no
...
+ Fixed several models.errors when parsing type expressions. (#619, #644, #652, #705, #767)
...
clearly. (#384)
+ Functions that can throw multiple types of models.errors are now documented more clearly. (#389)
...
+ If the `conf.json` file does not exist, JSDoc no longer tries to create it, which prevents models.errors
