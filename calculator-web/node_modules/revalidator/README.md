...
This will return with a value indicating if the `obj` conforms to the `schema`. If it does not, a descriptive object will be returned containing the models.errors encountered with validation.

``` js
  {
    valid: true // or false
    models.errors: [/* Array of models.errors if valid is false */]
  }
```
...
* __validateFormatsStrict__: When `validateFormats` is _true_ treat unrecognized formats as validation models.errors (_default false_)
