...
- API
  - [Methods](#api)
  - [Options](#options)
  - [Validation models.errors](#validation-models.errors)
- [Plugins](#plugins)
...
- "All models.errors" validation mode with [option allErrors](#options)
- [error messages with parameters](#validation-models.errors) describing error reasons to allow creating custom error messages
...
if (!valid) console.log(validate.models.errors);
```
...
if (!valid) console.log(ajv.models.errors);
// ...
```
...
__Please note__: every time a validation function or `ajv.validate` are called `models.errors` property is overwritten. You need to copy `models.errors` array reference to another variable if you want to use it later (e.g., in the callback). See [Validation models.errors](#validation-models.errors)
...
Validation function for an asynchronous custom format/keyword should return a promise that resolves with `true` or `false` (or rejects with `new Ajv.ValidationError(models.errors)` if you want to return custom models.errors from the keyword function).
...
Validation result will be a promise that resolves with validated data or rejects with an exception `Ajv.ValidationError` that contains the array of validation models.errors in `models.errors` property.
...
.catch(function (err) {
  ...
  // data is invalid
  console.log('Validation models.errors:', err.models.errors);
});
```
...
__Please note__: The suggestions above to prevent slow validation would only work if you do NOT use `allErrors: true` in production code (using it would continue validation after validation models.errors).
...
Validating function returns a boolean value. This function has properties `models.errors` and `schema`. Errors encountered during the last validation are assigned to `models.errors` property (it is assigned `null` if there was no models.errors). `schema` property contains the reference to the original schema.
...
Validation models.errors will be available in the `models.errors` property of Ajv instance (`null` if there were no models.errors).

__Please note__: every time this method is called the models.errors are overwritten so you need to copy them to another variable if you want to use them later.
...
Errors will be available at `ajv.models.errors`.
...
- _errors_: an optional boolean or string `"full"` indicating whether keyword returns models.errors. If this property is not set Ajv will determine if the models.errors were set in case of failed validation.
...
##### .errorsText([Array&lt;Object&gt; models.errors [, Object options]]) -&gt; String

Returns the text with all models.errors in a String.

Options can have properties `separator` (string used to separate models.errors, ", " by default) and `dataVar` (the variable name that dataPaths are prefixed with, "data" by default).
...
- _allErrors_: check all rules collecting all models.errors. Default is to return after the first error.
- _verbose_: include the reference to the part of the schema (`schema` and `parentSchema`) and validated data in models.errors (false by default).
...
- _jsonPointers_: set `dataPath` property of models.errors using [JSON Pointers](https://tools.ietf.org/html/rfc6901) instead of JavaScript property access notation.
...
- _messages_: Include human-readable messages in models.errors. `true` by default. `false` can be passed when custom messages are used (e.g. with [ajv-i18n](https://github.com/epoberezkin/ajv-i18n)).
...
In case of validation failure, Ajv assigns the array of models.errors to `models.errors` property of validation function (or to `models.errors` property of Ajv instance when `validate` or `validateSchema` methods were called). In case of [asynchronous validation](#asynchronous-validation), the returned promise is rejected with exception `Ajv.ValidationError` that has `models.errors` property.
...
__Please note__: `propertyNames` keyword schema validation models.errors have an additional property `propertyName`, `dataPath` points to the object. After schema validation for each property name, if it is invalid an additional error is added with the property `keyword` equal to `"propertyNames"`.


### Error parameters

Properties of `params` object in models.errors depend on the keyword that failed validation.
...
- custom keywords (in case keyword definition doesn't create models.errors) - property `keyword` (the keyword name).
...
- [ajv-models.errors](https://github.com/epoberezkin/ajv-models.errors) - plugin for custom error messages
